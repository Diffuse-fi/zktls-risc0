...
fn extract_pair_data<'a>(
    json_data: &'a JsonValue,
    pair_name: &'a String,
) -> (&'a String, u64, u64) {
    ...
    if integer_and_fractional.len() != 2 {
        eprintln!(
            "Error: 'price' value for {} is {}, unable to parse as float",
            pair_name, price_str
        );
    ...
    assert!(
        decimal_points <= 5,
        "price decimal points <= 5 are hardcoded"
    ); // TODO 5 hardcoded
    ...
    let timestamp = match json_data[pair_name]["closeTime"].as_u64() {
        ...
        None => {
            eprintln!(
                "Error: 'closeTime' value for {} pair is not a valid u64 or is missing",
                pair_name
            );
